...
DB_PATH = "keys.db"
UPDATE_AVAILABLE = False  # Indicateur de mise à jour
...
# Page d'accueil : Affiche "UPDATE IS AVAILABLE" si une mise à jour est disponible
@app.route("/", methods=["GET"])
def home():
    if UPDATE_AVAILABLE:
        return render_template_string("""
            <h1 style="color: red;">UPDATE IS AVAILABLE</h1>
            <p>A new version of the tool is available. Please download the latest version from the link below:</p>
            <a href="{{ update_link }}" target="_blank">Download New Version</a>
        """, update_link="https://example.com/download"), 200

    return jsonify({"message": "Welcome to the Advanced Activation Key API!"}), 200

# Endpoint pour activer/désactiver la mise à jour
...
def toggle_update():
    ...
    # Permet de basculer l'état de la mise à jour
    data = request.json
    UPDATE_AVAILABLE = data.get("update_available", False)
    status = "enabled" if UPDATE_AVAILABLE else "disabled"

    return jsonify({"message": f"Update mode {status}"}), 200
